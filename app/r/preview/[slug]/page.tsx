...
import { getRotatorPreview } from '@/lib/api/rotators';
...
export default function RotatorPreviewPage() {
  ...
  const slug = params?.slug as string | undefined; // Explicitly type slug
  ...
  useEffect(() => {
    if (!slug) {
      console.error('[Preview Error] Missing slug');
    ...
    const fetchLink = async () => {
      try {
        const res = await getRotatorPreview(slug);
        console.log('[RotatorPreviewPage] Slug:', slug);

...
export default function PublicVendorDirectory() {
    ...
    const [user, setUser] = useState<{ email?: string; role: string; user_id: string } | null>(null);
    ...
    useEffect(() => {
        ...
        router.replace(`/affiliate/vendors?${params.toString()}`);
                        ...
                        {filtered.map((vendor) => (
                            <tr key={vendor.id} className="border-t">
                                ...
                                <td className="p-2 flex flex-col gap-2">
                                    <Link
                                        href={`affiliate/vendors/${vendor.id}`}
                                        className="text-sm bg-black text-white text-center px-2 py-2 font-semibold rounded hover:bg-gray-800"
                                    >
                                        View
                                    </Link>
                                    <NegotiateModal vendorId={Number(vendor.id)} vendorName={vendor.company_name} />
